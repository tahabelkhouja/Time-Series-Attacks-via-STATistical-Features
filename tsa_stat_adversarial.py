...
def main(argv): 
    ...
    #Initializations
    adv_train = np.ndarray((0, 1, cnn_model.seg_size, cnn_model.channel_nb))
    clean_train = np.ndarray((0, 1, cnn_model.seg_size, cnn_model.channel_nb))
    ...
    for sample_id in range(len(X_in)):
        ...
        if y_s != FLAGS.t:
            clean_sample = tf.convert_to_tensor(X_s.reshape((1,1,FLAGS.window_size, FLAGS.channel_dim)))
            y_orig = cnn_model.predict(clean_sample).numpy()[0]
            ...
            Xadv, poly_transform[y_orig], _ = cnn_model.tsastat_attack(clean_sample, FLAGS.t, proj_fn_dict, a_init=poly_transform[y_orig],
                deg=FLAGS.deg, eta_init=5e-1, rho=FLAGS.rho, c=FLAGS.rho+1e-2, max_iter=FLAGS.max_iter, verbose=False)  
        
            adv_train = np.concatenate([adv_train, Xadv.numpy().reshape(1, 1, cnn_model.seg_size, cnn_model.channel_nb)])
            clean_train = np.concatenate([clean_train, clean_sample.numpy().reshape(1, 1, cnn_model.seg_size, cnn_model.channel_nb)])
                
            y_adv_train = np.concatenate([y_adv_train, [y_s]])
            target_label = np.concatenate([target_label, [FLAGS.t]])
...
if __name__=="__main__":    
    ...
    flags.DEFINE_integer('max_iter', 5000, 'Attack maximum number of iteratioon per example')
    ...
    flags.DEFINE_multi_integer('ft_cons_set', (0,1,2,3,4), 'Statistical features constraints')
